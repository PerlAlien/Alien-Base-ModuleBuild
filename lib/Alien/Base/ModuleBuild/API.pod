...
Allows the Build.PL script to set the ALIEN_INSTALL_TYPE variable. Note that if the variable is present,
it will not be overriden by this value. Possible values are 'system' and 'share'.
